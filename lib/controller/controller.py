...
class LibController(object):
    def __init__(self, script_path, arguments, output, dfh):
        global VERSION
        ...
        self.dfh.config_data(self.extensions, self.prefixes, self.suffixes, self.threads_count, self.dictionary, self.httpmethod)
    ...
    def append_error_log(self, path, error_msg):
        with self.threads_lock:
            ...
            self.error_log.write(os.linesep + line + "one")
            self.error_log.flush()

    # Handle CTRL+C. Since it is a library we don't want to handle it to much
